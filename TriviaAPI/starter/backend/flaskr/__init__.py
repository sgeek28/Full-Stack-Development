...
def paginate_questions_item(request, selection):
...
def create_app(test_config=None):
    ...
    def get_questions():
        ...
        current_questions = paginate_questions_item(request, selection)
    ...
    def post_question():
        ...
        # if no search term, create new question
            ...
            # load data from body
            new_question_item = body.get('question')
            new_answer_item = body.get('answer')
            new_difficulty_item = body.get('difficulty')
            new_category_item = body.get('category')

            # ensure all fields have data
            if ((new_question_item is None) or (new_answer_item is None)
                    or (new_difficulty_item is None) or (new_category_item is None)):
                abort(422)

            try:
                # create and insert new question
                question_item = Question(question=new_question_item, answer=new_answer_item,
                                    difficulty=new_difficulty_item, category=new_category_item)
                ...
                return jsonify({
                    ...
                    'question_created': question_item.question,
                    'questions': current_questions,
                    'total_questions': len(Question.query.all())
                })
